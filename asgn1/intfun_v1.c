 ...
 *
 * @param	numToPrint	The number to convert
 * @param	digitCount	Keeps count of bits for
 *						formatting
 *
 * -------------------------------------------- */
...
/*
	if (numToPrint == 0) {
		short numZeros = 0;
		while (numZeros < MAX_BITS - *digitCount) {
			printSpaces(numZeros);
			printf("0");
			numZeros++;
		}
		return;
	...
	*digitCount = *digitCount + 1;
	rPrintAsBinary(numToPrint / 2, digitCount);
	int bitToPrint = numToPrint % 2;
	printSpaces(*digitCount);
	printf("%d", bitToPrint);
	*digitCount = *digitCount - 1;
*/
}
...
/*	short digitCount = 0;
	short* pDigitCount = &digitCount;
	rPrintAsBinary(numToPrint, pDigitCount);
*/
}
